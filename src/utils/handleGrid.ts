...
export const _getCellChildNode = (cell: Cell): HTMLElement | null => {
    ...
    switch (cell._colInfo.dataType) {
        case 'text':
            ...
            childNode.classList.add(grid._id + '_data-value-text');
            childNode.innerText = cell._value;
            childNode.nType = 'text';
            break;
        case 'number':
            childNode = document.createElement('span');
            childNode.classList.add(grid._id + '_data-value-number');
            if (cell._value === null || cell._value === undefined || cell._value === grid._gridInfo.nullValue) childNode.innerText = grid._gridInfo.nullValue;
            else childNode.innerText = getFormatNumberFromCell(cell);
            childNode.nType = 'number';
            break;
        case 'mask':
            childNode = document.createElement('span');
            childNode.classList.add(grid._id + '_data-value-mask');
            childNode.innerText = cell._value;
            childNode.nType = 'mask';
            break;
        case 'date':
            childNode = document.createElement('span');
            childNode.classList.add(grid._id + '_data-value-date');
            if (cell._value === null || cell._value === undefined || cell._value === grid._gridInfo.nullValue) childNode.innerText = grid._gridInfo.nullValue;
        ...
        case 'month':
            childNode = document.createElement('span');
            childNode.classList.add(grid._id + '_data-value-month');
            if (cell._value === null || cell._value === undefined || cell._value === grid._gridInfo.nullValue) childNode.innerText = grid._gridInfo.nullValue;
        ...
        case 'select':
            if (Array.isArray(cell._value) && cell._value.length > 0) {
                ...
                childNode.classList.add(grid._id + '_data-value-select');
                childNode.addEventListener('change', function (e: any) { selectAndCheckboxOnChange(grid, e.target); });
                ...
                childNode = document.createElement('span');
                childNode.classList.add(grid._id + '_data-value-text');
                childNode.innerText = grid._gridInfo.nullValue;
                childNode.nType = 'text';
            }
            break;
        case 'checkbox':
            childNode = document.createElement('input');
            childNode.classList.add(grid._id + '_data-value-checkbox');
            childNode.addEventListener('change', function (e: any) { selectAndCheckboxOnChange(grid, e.target); });
            ...
            break;
        case 'button':
            if (cell._value === null || cell._value === undefined || cell._value === grid._gridInfo.nullValue) {
                childNode = document.createElement('span');
                childNode.classList.add(grid._id + '_data-value-text');
                childNode.innerText = grid._gridInfo.nullValue;
                childNode.nType = 'text';
                ...
                childNode.classList.add(grid._id + '_data-value-button');
                childNode.innerText = cell._value;
        ...
        case 'link':
            if (cell._value === null || cell._value === undefined || cell._value === grid._gridInfo.nullValue) {
                ...
                childNode.classList.add(grid._id + '_data-value-link');
        ...
        case 'code':
            childNode = document.createElement('span');
            childNode.classList.add(grid._id + '_data-value-code');
            childNode.innerText = cell._value;
            ...
            if(grid._vg.dataType) {
                Object.keys(grid._vg.dataType).forEach((key) => {
                    if(key === cell._colInfo.dataType) {
                        if(grid._vg.dataType[key].getChildNode) {
                            if(typeof grid._vg.dataType[key].getChildNode !== 'function') throw new Error('getChildNode must be a function.');
                            childNode = grid._vg.dataType[key].getChildNode(__getData(cell));
                            if(childNode) {
                        ...
                        childNode.classList.add(grid._id + '_data-value-' + key);
                        childNode.nType = key;
                    }
                });
            }
    ...
    childNode.classList.add(grid._id + '_data-value');
...
export const __gridCellReConnectedWithControlSpan = (cell: Cell) => {
    ...
    if(cell._rowSpan) {
        for(let row = cell._row + 1; row < cell._row + cell._rowSpan; row++) {
            __gridCellReConnectedWithControlSpan(_getCell(grid, row, cell._col)!);
        }
    }
    if(cell._colSpan) {
        for(let col = cell._col + 1; col < cell._col + cell._colSpan; col++) {
            __gridCellReConnectedWithControlSpan(_getCell(grid, cell._row, col)!);
        }
    }
};
